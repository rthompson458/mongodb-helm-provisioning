...
cleanup_local_pv() {
  ...
  if [[ -n "$target_claim" ]] && "${K[@]}" -n "$claim_ns" get pvc "$target_claim" >/dev/null 2>&1; then
    ...
    "${K[@]}" -n "$claim_ns" delete pvc "$target_claim" \
      --ignore-not-found=true \
      --wait=false >/dev/null

    if "${K[@]}" -n "$claim_ns" get pvc "$target_claim" >/dev/null 2>&1; then
      if ! "${K[@]}" -n "$claim_ns" wait \
        --for=delete \
        "pvc/$target_claim" \
        --timeout="$cleanup_timeout" >/dev/null 2>&1; then
  ...
  docker exec "${TC_STORAGE_NODE_NAME}" \
    rm -rf "${TC_STORAGE_BASE_PATH%/}/${pv}"
}
...
EOF
  ...
  cleanup_replica_set_storage)
    ...
    for ((i=0; i<TC_MEMBERS; i++)); do
      "${K[@]}" -n "${TC_NAMESPACE}" delete pvc \
        "data-${TC_DEPLOYMENT}-${i}" \
        --ignore-not-found=true \
    ...
    for ((i=0; i<TC_MEMBERS; i++)); do
      ...
      docker exec "${TC_STORAGE_NODE_NAME}" \
        rm -rf "${TC_STORAGE_BASE_PATH%/}/${pv}" || true
    done
    ;;
...
EOF
  ...
  release_deployment_lock)
    ...
    current_id=$(
      "${K[@]}" -n "${TC_NAMESPACE}" get configmap "${lock}" \
        -o jsonpath='{.data.operation_id}' 2>/dev/null || true
    )
    ...
    "${K[@]}" -n "${TC_NAMESPACE}" delete configmap \
      "${lock}" \
      --wait=true >/dev/null
    ;;
