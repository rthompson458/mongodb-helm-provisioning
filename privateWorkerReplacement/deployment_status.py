"""Read-only ReplicaSet, ShardedCluster, and shard status presentation.

Deployment mutation workflows live in deployments.py. This module owns only
customer-facing reads and tables. Keeping status presentation separate makes it
much easier for a reviewer to see that listing commands cannot accidentally
...
def _deployment_row(
    ...
    """Build one compact row used by the deployment inventory commands."""
...
def list_deployments(config: dict[str, Any], vault: VaultClient) -> None:
    """List all managed ReplicaSets and ShardedClusters in one table."""
...
def list_replica_sets(config: dict[str, Any], vault: VaultClient) -> None:
    """List only managed ReplicaSet deployments."""
...
def list_sharded_clusters(config: dict[str, Any], vault: VaultClient) -> None:
    """List only managed ShardedCluster deployments."""
...
def _status_count(item: dict[str, Any], lock: dict[str, Any] | None) -> int:
    """Return enough shard slots to display an in-progress add/delete operation."""
...
def _shard_status(
    ...
    """Combine live Kubernetes shard state with any active controller lock."""

    lock = read_deployment_lock(config, key)
    status = kube.sharded_cluster_status(
        config,
        key,
        _status_count(item, lock),
    )

    # The MongoDB CR's desired shard count can move before a new/removed
    # StatefulSet is visible. The lock gives us enough intent to show Creating,
    # Removing, or Removed instead of a confusing Unknown/absent row.
    if lock:
...
def _print_shards(
    ...
    """Print shard rows plus active-change/config-server/mongos status."""
...
def list_deployment(
    ...
    """Show detailed status for one ReplicaSet or ShardedCluster."""
...
def list_replica_set(
    ...
    """Show one ReplicaSet using the common detailed deployment view."""
...
def list_sharded_cluster(
    ...
    """Show one ShardedCluster using the common detailed deployment view."""
