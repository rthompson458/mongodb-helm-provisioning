...
This module owns deployment-level *mutation* policy:
...
- enforce the one-shard minimum; and
- protect topology changes with deployment locks.

Read-only deployment/shard presentation lives in deployment_status.py. Python
does not edit MongoDB CRs or persistent volumes directly. Every managed change
is expressed as desired state and passed to apply_inventory().
"""
