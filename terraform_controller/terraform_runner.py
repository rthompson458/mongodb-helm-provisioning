...
def _require(*names: str) -> None:
    ...
    if missing: raise ControllerError("Required executable(s) not found in PATH: " + ", ".join(missing))
...
def _sync(config: dict[str, Any]) -> Path:
    ...
    if not tfdir.is_dir(): raise ControllerError(f"Terraform subdirectory not found: {tfdir}")
    return tfdir


def apply_inventory(config: dict[str, Any], inventory: dict[str, dict[str, Any]]) -> None:
    _require("terraform", "git", "kubectl")
    _check_version()
    tfdir = _sync(config)
    token_name = config["vault_token_env"]
    token = os.getenv(token_name, "")
    if not token: raise ControllerError(f"Vault token environment variable '{token_name}' is not set.")
    env = os.environ.copy()
    env.update({
    ...
    if config["kube_context"]: init.append(f"-backend-config=config_context={config['kube_context']}")
    print("Initializing Terraform ...")
    run_process(init, cwd=tfdir, env=env)
    temp: Path | None = None
    try:
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", dir=tfdir, prefix=".tc.", suffix=".tfvars.json", delete=False) as handle:
            json.dump({"replica_sets": inventory}, handle, indent=2, sort_keys=True)
            handle.write("\n")
            temp = Path(handle.name)
        print("Applying Terraform ...")
        run_process(["terraform", "apply", "-input=false", "-auto-approve", f"-var-file={temp.name}"], cwd=tfdir, env=env)
    finally:
